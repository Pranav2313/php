...
<?php
	...
	if(isset($_SESSION['id']))
   {
	...
	$qry="select * from patients where UserID = '$patientid'";
	$sql=mysqli_fetch_array(mysqli_query($conn,$qry));
	$qry1="select count(*) from appointment where patientid = '$sql[0]'";
		$qry2="select count(*) from appointment where patientid = '$sql[0]' and status in('confirmed','pending') ";
  ...
  <main id="main">
	...
	min-height: 200px;
		...
		<form style="align-content:center;" action="patientgateway.php" method="post">
	...
	display: inline-block;">Edit</button>
			<button type="submit" name="add" style="border-radius: 20px;border: 1px solid #FFFFFF;
	background-color: #05D91D;
	color: #ffffff;
	font-size: 12px;
	font-weight: bold;
	padding: 10px 35px;
	...
	transition: 0.5s;
	transition: transform 80ms ease-in;
	display: inline-block;">Add Appointment</button>
